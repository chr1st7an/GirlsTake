...
//  GirlsTake
//
//  Created by Christian Rodriguez on 3/20/23.
//
...
struct DropDown: View{
    ...
    func RowView(_ title: String,_ size: CGSize)->some View{
        ...
        HStack{
            Image(systemName: "mappin.and.ellipse").padding(.trailing, -11).padding(.leading, 18)
            Text(title)
                ...
                if selection.value == title{
                    Rectangle()
                        .fill(activeTint)
                        .transition(.identity)
                        .frame(width: 330, height: size.height, alignment: .leading)
                }
            }
...
struct DropDownSelection : Equatable{
    var value: String
