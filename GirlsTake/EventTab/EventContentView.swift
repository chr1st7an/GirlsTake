...
struct EventContentView: View {
    ...
    var body: some View {
        NavigationStack(path: self.$eventViewModel.path){
    ...
    func EventListView()->some View{
        VStack{
            ForEach(eventViewModel.events, id: \.self){event in
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    ...
                    .frame(height: 220)
                    .overlay {
                        NavigationLink(value: EventStackModel.event(event)) {
                            VStack{
                                Text(event.title)
                                Text(event.location)
                                Text(event.address)
        ...
        }.navigationDestination(for: EventStackModel.self) {route in
            switch route {
...
//        EventContentView().environmentObject(EventViewModel(path: []))
//    }
//}
